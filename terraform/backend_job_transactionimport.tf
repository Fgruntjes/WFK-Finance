locals {
  cronjobs = [
    "institutionaccounttransactionimport"
  ]
}
resource "azurerm_linux_function_app" "cron_transactionimport" {
  for_each            = toset(local.cronjobs)
  name                = "v${var.app_environment}-cron-transactionimport"
  ...
  site_config {
    ...
    application_stack {
      docker {
        image_name   = "${data.azurerm_container_registry.app.login_server}/app.cron.${each.value}:${var.app_version}"
